...
export interface CommandPhraseReplaceTextOptions {
  duration?: number
  delay?: number
  occurrence?: number
}
...
export class CommandPhrase {
  ...
  constructor(
    ...
    const snapshot =
      typeof source === "string" ? snapshotCommandPhrase(source) : source
  ...
  *replaceText(
    tokenText: string,
    newText: string,
    options: CommandPhraseReplaceTextOptions = {},
  ) {
    ...
    yield* tokenRef().text(newText, options.duration ?? 0.22)

    this.tokens[match].text = newText
  }
  ...
  *restore(duration = 0.22) {
    yield* all(
      ...this.tokenRefs.map((tokenRef, index) =>
        all(
          tokenRef().fill(this.originalFill[index], duration),
          tokenRef().scale(1, duration),
        ),
      ),
    )
  ...
  *restyle(style: CommandPhraseRestyleOptions, duration = 0.35) {
    ...
    if (style.fontSize !== undefined) {
      ...
      animations.push(
        ...this.tokenRefs.map((tokenRef) =>
          tokenRef().fontSize(fontSize, duration),
        ),
      )
    }
    ...
    if (style.fill !== undefined) {
      ...
      animations.push(
        ...this.tokenRefs.map((tokenRef) => tokenRef().fill(fill, duration)),
      )
    }
