...
- `youtube-intro`
- `youtube-deep-dive`
- `backpropagation` — Markdown-first Backpropagation video, currently implementing Act I only.
- `backpropagation-basics` — older explainer covering perceptrons, deeper networks, and backpropagation; keep separate from the Markdown-first implementation.
...
Each substantial video should include a dedicated folder under `src/videos/<slug>/` with timestamped narration, production notes, and representative screenshots. For example, see `src/videos/backpropagation/` for the Markdown-first Act I implementation notes and `src/videos/backpropagation-basics/` for the older implementation.
