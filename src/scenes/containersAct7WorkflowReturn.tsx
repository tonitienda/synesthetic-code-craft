...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Layout ref={main} layout direction={'column'} gap={44} alignItems={'center'} justifyContent={'center'} width={'100%'} height={'100%'} padding={70} opacity={0}>
      <Title text={'Workflow returns'} sub={'sharper labels'} />
      <Layout layout direction={'column'} gap={38} alignItems={'center'}><Txt text={'build → image → push → registry → pull → local image → run → container'} fontSize={38} fill={c.text} /><Txt text={'image = layers + config · run = runtime setup · container = bounded process'} fontSize={32} fill={c.muted} /></Layout>
    </Layout>,
  );
