...
// A small labelled chip added to a container to make one namespace concrete —
// the "/" root a mount namespace exposes, or the interface a network one does.
...
export const playNamespaces = function* (world: World): ThreadGenerator {
  ...
  // arrive dimmed — present, but not yet the one we are looking at.
  ...
  // The shared read-only image is now a row of separate layer nodes. Pulse them
  // all so "same image underneath" is unmistakable.
  const sharedFlash: ThreadGenerator[] = (imageFs?.layers ?? []).map((layer) =>
    layer.node
      .stroke(containerColors.readonly, 0.4)
      .to(theme.primary.base + "99", 0.6),
  )
  ...
  // neither can see the other's. A dashed divider rises between the two
  // containers to make the "can't see across" boundary literal, while both
  // process status dots pulse.
  yield* caption.text(
  ...
  // ── NETWORK NAMESPACE ──────────────────────────────────────────────────────
  // Its own interfaces, routes and ports. Light the net badge and give each
  // container its own private interface + port.
  yield* caption.text(
  ...
  const hostRest = localSystem?.node.stroke() ?? theme.textMuted + "99"
  yield* all(
    ...badgesA.map((b) => light(b, true, 0.4)),
    ...badgesB.map((b) => light(b, true, 0.4)),
    localSystem
      ? localSystem.node.stroke(NS, 0.5).to(hostRest, 0.7)
      : waitFor(0),
  )
  yield* all(
  ...
  // Leave the caption behind before cgroups: its track rides lower in the host
  // panel, so the top strip is handed cleanly to the next idea.
