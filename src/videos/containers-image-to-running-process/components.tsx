...
export function Title({text, sub}: {text: string; sub?: string}) {
  return <Layout layout direction={'column'} gap={14} alignItems={'center'}>
    <Txt text={text} fontSize={58} fontWeight={700} fill={c.text} />
    {sub && <Txt text={sub} fontSize={28} fill={c.muted} />}
  </Layout>;
}

export function Card({title, body, color = c.cyan, width = 330}: {title: string; body?: string; color?: string; width?: number}) {
  return <Rect layout direction={'column'} gap={12} width={width} height={170} radius={24} padding={24} fill={c.panel} stroke={color} lineWidth={4}>
...
export function ImageBox({label = 'image'}: {label?: string}) {
  return <Rect layout direction={'column'} gap={10} width={260} height={190} radius={22} padding={22} fill={c.panelAlt} stroke={c.cyan} lineWidth={5}>
...
export function LayerStack({labels = ['base filesystem', 'packages', 'runtime deps', 'app files'], writable}: {labels?: string[]; writable?: string}) {
  return <Layout layout direction={'column'} gap={8} alignItems={'center'}>
    {writable && <Rect width={430} height={48} radius={12} fill={'#3b2d13'} stroke={c.amber} lineWidth={3}><Txt text={writable} fontSize={22} fill={c.text} /></Rect>}
    {labels.slice().reverse().map((label, index) => <Rect key={label} width={430 - index * 18} height={46} radius={12} fill={'#102033'} stroke={c.cyan} lineWidth={2} opacity={0.9}>
...
export function ProcessBox({label = 'process'}: {label?: string}) {
  return <Circle size={140} fill={'#172554'} stroke={c.green} lineWidth={6}>
    <Txt text={label} fontSize={28} fill={c.text} />
  </Circle>;
}

export function ContainerInstance({label = 'container'}: {label?: string}) {
  return <Rect layout direction={'column'} gap={14} width={360} height={290} radius={28} padding={22} fill={'#0b1220'} stroke={c.violet} lineWidth={5}>
...
export function KernelLayer() {
  return <Rect width={1000} height={90} radius={24} fill={'#1e293b'} stroke={c.stroke} lineWidth={4}>
