...
function usage() {
  console.error(
    "Usage: node scripts/capture-motion-canvas-frame.mjs <project-file> <timestamp-seconds> [output.png]",
  )
  console.error(
    "Example: node scripts/capture-motion-canvas-frame.mjs src/projects/containers-image-to-running-process.ts 35 artifacts/screenshots/containers/035s.png",
  )
}
...
const output = resolve(
  outputArg ??
    `artifacts/screenshots/${projectSlug}/${String(Math.round(seconds)).padStart(3, "0")}s.png`,
)
const browser = await findBrowser()
if (!browser) {
  console.error(
    "No Chromium-compatible browser found. Install chromium or set CHROME=/path/to/chrome.",
  )
  process.exit(1)
}
...
if (!Number.isInteger(port) || port <= 0) {
  console.error(
    `Invalid MOTION_CANVAS_PREVIEW_PORT: ${process.env.MOTION_CANVAS_PREVIEW_PORT}`,
  )
  process.exit(1)
}
const server = run(
